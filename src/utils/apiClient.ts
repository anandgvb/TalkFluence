...
class ApiClient {
  ...
  async generateFeedback(transcriptions: string[], questions: string[]): Promise<string> {
    try {
      const response = await this.client.post<{ response: string }>("/llmGenerate", {
        currentQuestion: "Generate detailed feedback for this interview based on the following transcriptions and questions:",
